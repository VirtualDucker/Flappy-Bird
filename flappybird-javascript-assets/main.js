...
function preload() { // this is for loading the media files for the game 
...
function is_collsion(im1,x1,y1, im2,x2,y2) {
...
function play() { // processing gameplay|runs 1/25th of a second
    //Pipes
    pipe[0] -= pipe_speed;
    ...
    //Detecting Collision
    if (is_collsion(BIRD, bird[0], bird[1], TOP_PIPE, pipe[0], pipe[1]-TOP_PIPE.height) || 
        is_collsion(BIRD, bird[0], bird[1], BOTTOM_PIPE, pipe[0], pipe[1]+pipe_gap)) {
        pipe[0] = game_size[0];
        pipe[1] = 50 + random(game_size[1] - 200);
        pipe_gap = 100 + random(100);
        lives -= 1;
...
function draw() {
    ...
    if (! game_running) {
        ...
        textSize(40);
        fill(255);
        text('Flappy Bird Remake', game_size[0]/2, game_size[1]/3);
        text('Press/Click to start', game_size[0]/2, game_size[1]*5/6);
        if (mouseIsPressed) {
            game_running = true;
            lives = 5;
            points = 0;
